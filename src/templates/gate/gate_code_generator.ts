...
/** Read the C1 expiry duration from chip config, with fallback. */
function getExpiryMs(selection: ChipSelection): number {
  const val = selection.chipConfigs.C1?.expiryDurationMs;
  const n = Number(val);
...
function generateTribePermitMove(selection: ChipSelection): string {
  ...
  const expiryMs = getExpiryMs(selection);
  const needsTribeConfig = hasChip('A1', selection);

  const tribeConfigBlock = needsTribeConfig
    ? `
...
`
  ...
  const adminBlock = needsTribeConfig
    ? `
// === Admin Functions ===
...
`
  ...
  // A1 reads expiry from on-chain config; A2/A3 use compile-time constant from C1
  const expiryLine = needsTribeConfig
    ? `let expiry_ms = tribe_cfg.expiry_duration_ms;`
    : `let expiry_ms: u64 = ${expiryMs};`;
...
const PERMIT_DURATION_MS: u64 = ${expiryMs};
...
`
  ...
  const tollConfigBlock = needsPayment
    ? `
public struct TollConfig has drop, store {
    price: u64,
    owner_address: address,
...
#[error(code = 5)]
const EExpiryOverflow: vector<u8> = b"Expiry timestamp overflow";
${tribeConfigBlock}${tollConfigBlock}
public fun issue_jump_permit(
    extension_config: &ExtensionConfig,
    source_gate: &Gate,
    destination_gate: &Gate,
